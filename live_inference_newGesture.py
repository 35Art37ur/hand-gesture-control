...
Zusaetzlich unterstuetzt: 'faust' als Ruhe-/Pause-Geste. Wird eine Faust
erkannt, wird bewusst KEINE Aktion ausgefuehrt -- so kann man die Erkennung
gezielt "stummschalten", ohne die Hand aus dem Bild nehmen zu muessen.
...
SEQUENZ_LAENGE = 20
SUBSAMPLE_SCHRITT = 2
ROH_PUFFER_LAENGE = SEQUENZ_LAENGE * SUBSAMPLE_SCHRITT
...
def fuehre_aktion_aus(geste_name):
    if geste_name == "faust":
        # Ruhe-/Pause-Geste: bewusst KEINE Aktion, keine Tastatur-/Maus-Ausgabe.
        # Dient dazu, die Erkennung gezielt zu "stummschalten", wenn man
        # gerade keine Geste ausfuehren will (Hand einfach zur Faust ballen).
        print(">>> Faust erkannt: keine Aktion (Ruhe-/Pause-Geste)")
        return
    elif geste_name == "wischen_rechts":
